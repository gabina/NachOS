...
#include "utility.hh"
#include "../userprog/openFileTable.hh"
...
public:
    ...
    /// Initialize a `Thread`.
    /// Se le debe pasar un puntero a un puerto del padre en caso de querer hacer Join
    /// Por defecto, no se hace Join y el puntero es NULL
    Thread(const char *debugName, Port * dadPort = NULL, int prio = 4);
    ...
    /// The thread is done executing. Manda un mensaje en caso de haberse hecho join.
    void Finish(int status = 0);

	/// Join made by Gabina. Retorna el status 
	int Join();
...
private:
	...
	/// Real priority -> la prioridad real del thread, aunque momentaneamente se esté utilizando otra
	int realPriority;
	
	///Tabla de archivos abiertos para el thread
	OpenFileTable* threadTable;
